...
class LamaHDataset(Dataset):
    ...
    def __init__(self, root_dir, years=range(2000, 2018), root_gauge_id=399, rewire_graph=True,
        ...
        # 动态添加反向边及对应的特征
        self.edge_index, self.edge_attr = self.add_reverse_edges_for_no_input(self.edge_index, self.edge_attr)
    ...
    #
    #     # 动态添加反向边及对应的 edge_attr
    #     edge_index, edge_attr = self.add_reverse_edges_for_no_input(self.edge_index, self.edge_attr)
    #
    #     return Data(x=x, y=y.unsqueeze(-1), edge_index=edge_index, edge_attr=edge_attr)
    ...
    def add_reverse_edges_for_no_input(self, edge_index, edge_attr):
        """
        对没有输入的点添加从下游点来的反向边，并更新 edge_attr。
        参数:
            edge_index (torch.Tensor): 形状为 [2, num_edges] 的边索引张量。
            edge_attr (torch.Tensor): 形状为 [num_edges, edge_attr_dim] 的边属性张量。
        返回:
            Tuple[torch.Tensor, torch.Tensor]: 修改后的边索引张量和边属性张量。
        """
        source_nodes = edge_index[0].tolist()
        target_nodes = edge_index[1].tolist()
    
        # 找到没有输入的点
        nodes_without_input = set(source_nodes) - set(target_nodes)
        ...
        for node in nodes_without_input:
            # 找到该节点的目标节点
            downstream_nodes = [target for src, target in zip(source_nodes, target_nodes) if src == node]
            for downstream_node in downstream_nodes:
                new_edges.append([downstream_node, node])  # 添加反向边
                # 使用原边的属性作为反向边的属性
                original_edge_idx = source_nodes.index(node)  # 找到原始边的索引
                new_attrs.append(edge_attr[original_edge_idx].tolist())
    
        # 将新边及其属性添加到原始边集合中
        if new_edges:
            reverse_edges = torch.tensor(new_edges, dtype=torch.long).t()
            edge_index = torch.cat([edge_index, reverse_edges], dim=1)
    
            reverse_attrs = torch.tensor(new_attrs, dtype=torch.float)
            edge_attr = torch.cat([edge_attr, reverse_attrs], dim=0)
